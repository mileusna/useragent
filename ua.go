...
var mobileStrings = []string{"Mobile", "Mobile Safari", "iPhone"}
...
func Parse(userAgent string) UserAgent {
	...
	case tokens.exists("Googlebot"):
		...
		ua.Bot = true
		ua.Mobile = tokens.existsAny(mobileStrings...)
...
func (p properties) existsAny(keys ...string) bool {
	for _, k := range keys {
		for _, prop := range p.list {
			if strings.Contains(strings.ToLower(prop.Key), strings.ToLower(k)) {
				return true
			}
		}
	}
